...
export class Admin {
    ...
    newProyect(title, desc) {
        this.projects.push(new Proyect(this.idCounter,title, desc))
        this.idCounter++
    }
    getProyect(id){
    ...
    addTask(proyectId, title, desc, priority,finishied){
        this.getProyect(proyectId).addTask(title,desc,priority,finishied)
    }
    ...
    loadProyects(proyects){ //Load the proyects from String
        ...
        list.forEach(p => {
            const pItem = p.split(/§|¦/)
            this.newProyect(pItem[0],pItem[1],pItem[2])
            for (let i = 3; i < pItem.length; i+=4) {
                this.addTask(this.idCounter-1, pItem[i], pItem[i+1], pItem[i+2], pItem[i+3])
            }
        })
    }
}